...
impl ResponseTime {
    pub fn increment_time_by(&self, instant: std::time::Instant) {
        let elapsed_millis = instant.elapsed().as_millis() as u64;
        self.0.record(elapsed_millis);
    }

    pub fn increment_time_by_duration(&self, duration: std::time::Duration) {
        let elapsed_millis = duration.as_millis() as u64;
        self.0.record(elapsed_millis);
    }
